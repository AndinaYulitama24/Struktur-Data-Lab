#include<bits/stdc++.h> // buat soal sendiri perhitungannya dan gabar menggunakan bellmarfort
using namespace std;
...
void bellmanFord(vector<edge> e, int nV, int source){
    ...
    for(int i = 0; i < nV- 1; i++){
        for(int j = 0; j< e.size(); j++){
            ...
            // skip kalau src belum ada nilai distance
            if(distance[src] == INT_MAX) continue;

            // uptade distance kalo ketemu yang lebih kecil
