...
void dijkstra(int V, vector<vector<iPair>> &adj, int src) {
    ...
    priority_queue<iPair, vector<pair>, greater<iPair>>pq;

    dist[src] = 0;
    pq_push ({0, src}); // (jarak simpul)
    ...
    while(!pq.empty()){
        ...
        // jika jaraj yang diambil bukan terkecil, skip
        is(d > dist[u]) continue;

        // periksa semua tentangga dari simpul u
        for ( auto edge :adj[u]){
            int v = edge.first; // vertex tetangga
            int w= edge.second; // bobot dari u ->

            // jika jarak melalui u lebih kecil, update jarak  ke u
            if(dist[u] + w < dist[v]){
                dist[v] = dist[u] + w;
                pq.push ({dist[v], v});
            }
